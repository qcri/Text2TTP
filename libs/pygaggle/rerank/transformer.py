...
from libs.pygaggle.model import (BatchTokenizer,
...
__all__ = ['MonoT5',
           ...
           'CrossEncoderTransformersReranker']
...
class UnsupervisedTransformerReranker(Reranker):
    ...
    def __init__(self,
                 ...
                 model_max_length: Optional[str] = None,
                 method: str = 'max',
        ...
        if model_max_length is None:
            assert hasattr(tokenizer.tokenizer, 'model_max_length'), "could'nt find model max length"
            model_max_length = tokenizer.tokenizer.model_max_length

        self.model = model
        self.tokenizer = tokenizer
        self.encoder = LongBatchEncoder(model, tokenizer, model_max_length)
...
class SentenceTransformersReranker(Reranker):
    def __init__(self,
        ...
        device = device or ('cuda' if torch.cuda.is_available() else 'cpu')
        if model_max_length is None:
            assert hasattr(tokenizer.tokenizer, 'model_max_length'), "Couldn't find model max length"
            model_max_length = tokenizer.tokenizer.model_max_length

        self.model = SentenceTransformer(pretrained_model_name_or_path, device=device)
        self.tokenizer = SimpleBatchTokenizer(self.model.tokenizer, batch_size=batch_size)
        # self.encoder = LongBatchEncoder(self.model, self.tokenizer, model_max_length)

    @torch.no_grad()
    def rescore(self, query: Query, texts: List[Text]) -> List[Text]:
        encoded_query = self.model.encode([query.text])
        encoded_documents = self.model.encode([text.text for text in texts])
        
        sim = cos_sim(encoded_query, encoded_documents)[0]

        texts = deepcopy(texts)
        for score, text in zip(sim, texts):
            text.score = score
...
class CrossEncoderTransformersReranker(Reranker):
    def __init__(self,
        ...
        self.model = CrossEncoder(
            pretrained_model_name_or_path, max_length=model_max_length, device=device
        )
