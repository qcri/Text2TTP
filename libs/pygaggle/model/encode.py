...
from libs.pygaggle.rerank.base import TextType
...
class LongBatchEncoder:
    ...
    def encode(self, batch_input: List[TextType]) -> EncoderOutputBatch:
        ...
        for ret in self.tokenizer.traverse(batch_input):
            ...
            rest_input_ids = [(idx, x[max_len:]) for idx, x in input_ids]
            while new_input_ids:
                attn_mask = [[1] * len(x[1]) +
                             [0] * (max_len - len(x[1]))
                ...
                if isinstance(self.encoder, SentenceTransformer):
                    outputs = self.encoder(dict(input_ids=new_input_ids,
                                           attention_mask=attn_mask))['sentence_embedding']
                else:
                    outputs = self.encoder(input_ids=new_input_ids,
                ...
                new_input_ids = [(idx, x[:max_len]) for idx, x in rest_input_ids if len(x) > max_len]
                max_len = min(max(map(lambda x: len(x[1]), new_input_ids),
                                  default=0), self.msl)
                rest_input_ids = [(idx, x[max_len:]) for idx, x in rest_input_ids]
